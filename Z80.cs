...
#if (Z80DEBUG)
            string Z80DEBUG_target;
            if (useHL)
                if (useIX) Z80DEBUG_target = $"(IX{d:+0;-#})";
                else Z80DEBUG_target = useIY ? $"(IY{d:+0;-#})" : "(HL)";
            else
                Z80DEBUG_target = useIX ? $"(IX{d:+0;-#}), {RName(lo)}" : useIY ? $"(IY{d:+0;-#}), {RName(lo)}" : RName(lo);
#endif
            switch (hi)
            {
                case 0:
...
#if (Z80DEBUG)
        private static bool Z80DEBUG_atStart = true;
